...
class FxThread(ThreadJob):
    ...
    def ccy_amount_str(self, amount, commas, prec=None):
        if prec is None:
            prec = 2
        prec = CCY_PRECISIONS.get(self.ccy, prec)
    ...
    def get_fiat_status_text(self, btc_balance, base_unit, decimal_point):
        ...
        return _("  (No FX rate available)") if rate.is_nan() else " 1 %s~%s %s" % (base_unit,
            self.value_str(COIN / (10**(8 - decimal_point)), rate, 6), self.ccy)

    def fiat_value(self, satoshis, rate):
        return Decimal('NaN') if satoshis is None else Decimal(satoshis) / COIN * Decimal(rate)

    def value_str(self, satoshis, rate, prec=None):
        return self.format_fiat(self.fiat_value(satoshis, rate), prec)

    def format_fiat(self, value, prec=None):
        if value.is_nan():
            return _("No data")
        return "%s" % (self.ccy_amount_str(value, True, prec))
