...
from electrum_pac.dash_ps import (PSPossibleDoubleSpendError,
                                   PSSpendToPSAddressesError)
from electrum_pac.storage import WalletStorage, StorageReadWriteError
from electrum_pac.wallet_db import WalletDB
from electrum_pac.wallet import Wallet, InternalAddressCorruption, Abstract_Wallet
from electrum_pac.plugin import run_hook
from electrum_pac import util
from electrum_pac.util import (profiler, InvalidPassword, send_exception_to_crash_reporter,
                           format_satoshis, format_satoshis_plain, format_fee_satoshis)
from electrum_pac.invoices import PR_PAID, PR_FAILED
from electrum_pac import blockchain
from electrum_pac.network import Network, TxBroadcastError, BestEffortRequestFailed
from electrum_pac.interface import PREFERRED_NETWORK_PROTOCOL, ServerAddr
...
from electrum_pac.util import (NoDynamicFeeEstimates, NotEnoughFunds,
                                DASH_BIP21_URI_SCHEME, PAY_BIP21_URI_SCHEME)

if TYPE_CHECKING:
    from . import ElectrumGui
    from electrum_pac.simple_config import SimpleConfig
    from electrum_pac.plugin import Plugins
...
class ElectrumWindow(App, Logger):
    ...
    def init_ui(self):
        ...
        if self.testnet:
            self.icon = os.path.dirname(KIVY_GUI_PATH) + "/icons/electrum-pac-testnet.png"
        else:
            self.icon = os.path.dirname(KIVY_GUI_PATH) + "/icons/electrum-pac.png"
    ...
    def get_max_amount(self, is_ps=False):
        from electrum_pac.transaction import PartialTxOutput
