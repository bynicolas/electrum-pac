...
from electrum_pac.i18n import _, languages
from electrum_pac.util import FileImportFailed, FileExportFailed, make_aiohttp_session, resource_path
from electrum_pac.invoices import PR_UNPAID, PR_PAID, PR_EXPIRED, PR_INFLIGHT, PR_UNKNOWN, PR_FAILED, PR_ROUTING
...
class ColorScheme:
    ...
    GREEN = ColorSchemeItem("#186e30", "#8af296")
    # YELLOW = ColorSchemeItem("#897b2a", "#ffff00")
    YELLOW = ColorSchemeItem("#b39b00", "#ffff00")
    # RED = ColorSchemeItem("#7c1111", "#f18c8c")
    RED = ColorSchemeItem("#ab0d0a", "#f18c8c")
    BLUE = ColorSchemeItem("#123b7c", "#8cb3f2")
    # DEFAULT = ColorSchemeItem("#818181", "white")
    DEFAULT = ColorSchemeItem("#6e6e6e", "white")
...
def export_meta_gui(electrum_window, title, exporter):
    ...
    filename = electrum_window.getSaveFileName(_("Select file to save your {}").format(title),
                                               'electrum_pac/_{}.json'.format(title), filter_)
    if not filename:
        return
    try:
        exporter(filename)
