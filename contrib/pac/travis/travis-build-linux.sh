...
docker run --rm \
    -v $(pwd):/opt \
    -w /opt/electrum-pac \
    -t zebralucky/electrum-pac-winebuild:Linux40x \
    /opt/electrum-pac/contrib/build-linux/sdist/build.sh
...
docker run --rm \
    -v $(pwd):/opt \
    -w /opt/electrum-pac/contrib/build-linux/appimage \
    -t zebralucky/electrum-pac-winebuild:AppImage40x ./build.sh


BUILD_DIR=/root/build
TOR_PROXY_VERSION=0.4.2.6
TOR_PROXY_PATH=https://github.com/zebra-lucky/tor-proxy/releases/download
TOR_DIST=electrum-pac/dist/tor-proxy-setup.exe
...
docker run --rm \
    ...
    -w $BUILD_DIR/electrum-pac \
    -t zebralucky/electrum-pac-winebuild:Wine40x \
    $BUILD_DIR/electrum-pac/contrib/build-wine/build.sh
