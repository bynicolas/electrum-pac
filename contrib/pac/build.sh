...
BUILD_DIST_DIR=build/electrum-pac/dist
BUILD_BIN_DIR=build/electrum-pac/bin
NAME=Dash-Electrum
TNAME=Pac-Electrum-Testnet
DEB_NAME=electrum-pac
APK_NAME=electrum_pac/
APK_TNAME=electrum_pac/_Testnet
...
if [[ -n $IS_RELEASE ]]; then
    echo electrum-pac version is $DASH_ELECTRUM_VERSION, release build
else
    echo electrum-pac version is $DASH_ELECTRUM_VERSION
fi
mkdir -p dist
